...
namespace TLDAG.Libraries.Core.IO
{
    ...
    public class StringStream : StreamBase
    {
        public StringStream(Stream stream) : base(stream) { }
        ...
        public void Read(string[] values, int offset, int count)
        {
            if (count == 0) return;
        ...
        public void Write(string[] values, int offset, int count, bool writeCount = false)
        {
            if (writeCount) WriteCount(count);
